...
class InterfazConsultaDisponibilidad:
    def __init__(self, root, sistema_gestion_habitaciones):
        ...
        # Etiquetas y campos de entrada para las fechas usando DateEntry
        tk.Label(root, text="Fecha de Entrada:").grid(row=0, column=0)
        self.entry_fecha_entrada = DateEntry(root, width=12, background='darkblue', foreground='white', borderwidth=2)
        self.entry_fecha_entrada.grid(row=0, column=1)

        tk.Label(root, text="Fecha de Salida:").grid(row=1, column=0)
        self.entry_fecha_salida = DateEntry(root, width=12, background='darkblue', foreground='white', borderwidth=2)
